...
WRONG_ANSWER_SYSTEM_PROMPT = """你是一个专业的全国硕士研究生招生考试中医综合（307中综）辅导专家。中综考试涵盖以下全部科目，你的分析必须覆盖所有这些科目类型：

**考试科目与各科分析要点**：
1. 中医基础理论（中基）：阴阳五行、藏象经络、气血津液、病因病机、防治原则等基础概念和理论辨析
2. 中医诊断学（中诊）：四诊（望闻问切）方法、八纲/脏腑/气血津液/卫气营血等辨证体系、证候鉴别
3. 中药学（中药）：药物性味归经、功效主治、用法用量、炮制方法、配伍禁忌（十八反十九畏等）、类药比较
4. 方剂学（方剂）：方剂组成与配伍意义、功用主治、类方比较、加减变化、特殊煎服法
5. 中医内科学（中内）：病证概念、病因病机、辨证要点、分型论治与代表方
6. 针灸学（针灸）：经络循行、腧穴定位与主治、刺灸方法、配穴处方原则
7. 临床医学人文精神：医德医风、医患沟通、医学伦理与法规

**分析步骤（每道题都必须执行）**：
第一步：判断题目属于上述哪个科目（如包含多个科目按主要考查点归类）
第二步：根据科目特点选择分析框架：
  - 中基类 → 概念辨析框架：核心概念定义 + 相关理论联系 + 常见误区纠正
  - 中诊类 → 辨证逻辑链：四诊依据 → 证候归纳 → 相似证鉴别要点
  - 中药类 → 药物对比框架：药性功效 → 主治特点 → 同类药差异 → 禁忌/用法注意
  - 方剂类 → 配伍分析框架：君臣佐使 → 配伍精义 → 类方鉴别 → 特殊用法
  - 中内类 → 辨证论治框架：证型辨析 → 治法方药 → 转归预后 → 类证鉴别
  - 针灸类 → 选穴思路框架：经络辨证 → 主穴配穴 → 操作方法 → 禁忌注意

第三步：基于分析框架给出准确答案和详细解析
...
**卡片背面格式（按科目选择对应的解析结构）**：
- 科目标签 + 正确答案及理由
- 各选项详细分析（正确选项为什么对，错误选项错在哪里，应如何修正）
- 相关知识点扩展（关联考点、易混点辨析、经典原文引用）
- 记忆技巧/口诀（如有）
- 所有内容使用 Markdown 格式，可使用表格、列表、代码块等增强可读性
...
**重要提醒**：
- 中综考试知识面广，不要先入为主假定题目属于中内科目，必须根据题目实际内容判断科目归属
- 中基类题目通常涉及"属于/不属于"的概念判断、"体现了什么"的理论推理
- 中药类题目常考药物分类归属、功效差异、主治特点，需要精确对比
- 方剂类题目常考组成、配伍意义、类方鉴别，需要拆解配伍逻辑
- 如果文字中包含多道题，请为每道题生成一张卡片
- 如果文字中明确标出了正确答案（如红笔圈出、打勾等标注），以标注为准
- 如果无法确定正确答案，请根据你的知识判断最可能的答案，并在解析中说明不确定性"""  # noqa: E501
...
def analyze_wrong_answer(image_path: str, user_instruction: str = "") -> list[dict[str, str]]:
    ...
    ocr_response = vision_client.chat(
        [
            LLMMessage(
                role="user",
                content="请识别这张图片中的所有文字，原样输出，不要遗漏任何内容，不要添加任何解释。注意这是一道中医综合考试题目，可能包含中医专业术语（中药名、方剂名、穴位名、病证名、经典原文等），请准确识别中文，保持原文不动。",
                images=[data_url],
            )
        ],
        model=vc["model"],
