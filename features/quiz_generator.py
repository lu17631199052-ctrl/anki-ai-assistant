...
QUIZ_SYSTEM_PROMPT = """你是出题专家。根据提供的知识点生成高质量选择题，帮助学习者区分容易混淆的概念。

【出题原则】
1. 根据内容自动判断学科领域，生成符合该学科特点的题目
2. 重点出对比/鉴别型题目：两个相似概念/条目的关键区别
3. 约60%对比型（两概念对比），约40%单一知识点题
4. 避免死记硬背的定义默写，考察理解和区分能力
5. 错误选项必须来自同一知识体系，有合理的干扰性
6. 解析简洁清晰：正确选项为何对 + 错误选项分别错在哪
  ...
  "questions": [
    {
      ...
      "选项D": "...",
      "正确答案": "A",
      "解析": "正确选项为何对，以及各错误选项的纠正"
    }
  ]
}
```
正确答案只能是 A/B/C/D 之一。题干具体明确。选项简洁。"""
...
def build_quiz_prompt(
    ...
    """Build compact, domain-agnostic prompt from grouped notes."""
    # Build compact summary: one line per item, all fields included
    lines = []
    for i, (name, fields) in enumerate(grouped_notes.items(), 1):
        parts = [name]
    ...
    type_instruction = {
        "differentiating": "重点生成对比型题目（区分两个易混淆概念）。",
        "single_disease": "重点生成单一知识点题目。",
        "mixed": "对比型约60%，单一知识点约40%。",
    }.get(question_type, "")

    user_content = (
        f"以下为 {len(grouped_notes)} 个知识点的内容。"
        f"请生成 {num_questions} 道选择题。{type_instruction}\n\n{all_content}"
    )
